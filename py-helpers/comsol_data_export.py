...
# this is a quick fix which should be updated
MAX_TOTAL_PATH_LENGTH_LIMIT = 250

def make_safe_output_filename(parent_folder: pathlib.Path,
                              stem: str,
                              suffix: str,
                              max_total_length: int = MAX_TOTAL_PATH_LENGTH_LIMIT) -> str:
    """
    ...
    ATTENTION: The way the path is shortend could use some rework. The current version distroys human readability pretty much as soon as there is the danger of running in the Windows Path limit.
    TIP: Use a high enought directory to avoid this comming into action. OR use Linux and increase global constant 'comsol_data_export.MAX_TOTAL_PATH_LENGTH_LIMIT' (default = 250).
